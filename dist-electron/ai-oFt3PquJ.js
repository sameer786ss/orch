...
const require_dist = require("./dist-xtXPF7Eb.js");
...
var tvly = require_dist.tavily({ apiKey: process.env.TAVILY_API_KEY || "" });
var CHAT_MODEL = "z-ai/glm4.7";
var EMBED_MODEL = "nvidia/nv-embedqa-e5-v5";
var DEFAULT_TOOL_TIMEOUT_MS = 3e4;
var MAX_TOOL_TIMEOUT_MS = 12e4;
var TOOL_OUTPUT_BUFFER_BYTES = 2 * 1024 * 1024;
function isPathInside(candidatePath, rootPath) {
	const relative = node_path.relative(rootPath, candidatePath);
	return relative === "" || !relative.startsWith("..") && !node_path.isAbsolute(relative);
}
function requireWorkspaceRoot(workspaceRoot) {
	if (!workspaceRoot) throw new Error("This tool requires an open workspace.");
	return workspaceRoot;
}
function resolveWorkspacePath(workspaceRoot, candidatePath) {
	const resolved = node_path.isAbsolute(candidatePath) ? node_path.resolve(candidatePath) : node_path.resolve(workspaceRoot, candidatePath);
	if (!isPathInside(resolved, workspaceRoot)) throw new Error("Tool path is outside the active workspace.");
	return resolved;
}
...
function assertSafeShellCommand(command) {
	if ([
		...
		/\brm\s+-rf\s+\/$/i,
		/\brm\s+-rf\s+\/\s+/i
	].some((p) => p.test(command))) throw new Error("Blocked potentially dangerous command.");
}
function sanitizeIncludeGlob(input) {
...
function buildSystemPrompt(workspacePath) {
	...
	return [
		...
		"",
		`## Workspace`,
		workspacePath ? `Current workspace root: ${workspacePath}\nAll relative paths resolve from this directory.` : "No workspace open. Use absolute paths or ask the user to open a folder.",
		"",
		"## Your Tools",
		"",
...
var TOOLS = [
	{
		...
		function: {
			name: "web_search",
			description: "Search the web for up-to-date information. Use this for docs, packages, APIs, error messages, best practices, or any real-world knowledge.",
			parameters: {
				type: "object",
				properties: {
					query: {
	...
	},
	{
		...
		function: {
			name: "run_command",
			description: "Execute a shell command in the workspace directory. Use to run builds, tests, install packages, git commands, etc. Captures stdout and stderr.",
			parameters: {
				type: "object",
				properties: {
					command: {
	...
	try {
		switch (name) {
			case "web_search": {
				...
				return (result.answer ? `Summary: ${result.answer}\n\n` : "") + result.results.map((r, i) => `[${i + 1}] ${r.title}\nURL: ${r.url}\n${r.content?.slice(0, 600)}`).join("\n\n---\n\n");
			}
			case "read_file": {
				const filePath = resolveWorkspacePath(requireWorkspaceRoot(workspaceRoot), String(args.path ?? ""));
				console.log("[AI] read_file:", filePath);
				return await node_fs_promises.readFile(filePath, "utf-8");
			}
			case "write_file": {
				const filePath = resolveWorkspacePath(requireWorkspaceRoot(workspaceRoot), String(args.path ?? ""));
			...
			case "list_directory": {
				const dirPath = resolveWorkspacePath(requireWorkspaceRoot(workspaceRoot), String(args.path ?? ""));
				console.log("[AI] list_directory:", dirPath);
				return (await node_fs_promises.readdir(dirPath, { withFileTypes: true })).map((e) => `${e.isDirectory() ? "DIR " : "FILE"} ${e.name}`).join("\n") || "(empty directory)";
			}
			case "run_command": {
				const root = requireWorkspaceRoot(workspaceRoot);
				const command = String(args.command ?? "").trim();
				if (!command) return "run_command requires a non-empty command.";
				...
				try {
					const { stdout, stderr } = await execFileAsync("/bin/sh", ["-c", command], {
						cwd,
						timeout: timeoutMs,
						maxBuffer: TOOL_OUTPUT_BUFFER_BYTES
					});
			...
			case "search_codebase": {
				...
				if (!query) return "search_codebase requires a non-empty query.";
...
async function runAgentChat(event, conversationId, messages, workspacePath) {
	console.log("[AI] runAgentChat — Kimi-K2 via NVIDIA NIM, convId:", conversationId);
	const workspaceRoot = workspacePath ? node_path.resolve(workspacePath) : null;
	const history = [{
		role: "system",
	...
	for (let iteration = 0; iteration < MAX_ITERATIONS; iteration++) {
		...
		try {
			...
			for await (const chunk of stream) {
				...
				if (delta.tool_calls) for (const tcDelta of delta.tool_calls) {
					const idx = tcDelta.index ?? 0;
					if (!toolCallAcc[idx]) toolCallAcc[idx] = {
						name: tcDelta.function?.name ?? "",
						args: ""
					};
					else if (tcDelta.function?.name && !toolCallAcc[idx].name) toolCallAcc[idx].name = tcDelta.function.name;
			...
			for (let i = 0; i < toolCalls.length; i++) {
				...
				const tcId = assistantMsg.tool_calls[i].id;
				let parsedArgs = {};
				try {
...
async function getEmbedding(text) {
	try {
		return (await nim.embeddings.create({
			model: EMBED_MODEL,
			input: text,
...
async function indexFile(workspace, filePath) {
	try {
		const content = await node_fs_promises.readFile(filePath, "utf-8");
		if (content.length > 2e5) return;
		const hash = (0, node_crypto.createHash)("md5").update(content).digest("hex");
		if ((await require_db.dbGetIndexedFile(workspace, filePath))?.content_hash === hash) return;
		const fileId = (0, node_crypto.createHash)("md5").update(`${workspace}:${filePath}`).digest("hex");
		await require_db.dbSaveIndexedFile(fileId, workspace, filePath, hash);
		const rawChunks = chunkText(content, 500, 50);
		const batch = [];
		for (let i = 0; i < rawChunks.length; i++) {
			const emb = await getEmbedding(rawChunks[i]);
			if (emb.length) batch.push({
				idx: i,
				text: rawChunks[i],
				emb
			});
		}
		if (batch.length) await require_db.dbSaveEmbeddingsBatch(fileId, batch);
	} catch {}
}
