...
var require_sql_wasm = /* @__PURE__ */ require_chunk.__commonJSMin(((exports, module) => {
...
var import_sql_wasm = /* @__PURE__ */ require_chunk.__toESM(require_sql_wasm());
var SQL;
var db;
var dbPath;
...
async function getDb() {
	if (db) return db;
	SQL = await (0, import_sql_wasm.default)({ locateFile: (file) => {
		try {
			return require.resolve(`sql.js/dist/${file}`);
		} catch {
			return node_path.join(process.resourcesPath ?? __dirname, "..", "node_modules", "sql.js", "dist", file);
		}
	} });
	dbPath = node_path.join(electron.app.getPath("userData"), "orch.db");
	const isNew = !node_fs.existsSync(dbPath);
	db = isNew ? new SQL.Database() : new SQL.Database(node_fs.readFileSync(dbPath));
	initSchema();
	if (isNew) persist();
	return db;
}
...
function persist() {
	if (!db || !dbPath) return;
	const buf = db.export();
	node_fs.writeFileSync(dbPath, Buffer.from(buf));
}
...
async function dbGetConversations() {
	const rows = (await getDb()).exec("SELECT id, title, status, created_at, updated_at FROM conversations ORDER BY updated_at DESC");
...
async function dbGetMessages(conversationId) {
	return dbGetMessagesSync(await getDb(), conversationId);
}
function dbGetMessagesSync(d, conversationId) {
	const rows = d.exec("SELECT * FROM messages WHERE conversation_id = ? ORDER BY timestamp ASC", [conversationId]);
	if (!rows.length) return [];
	const cols = rows[0].columns;
	return rows[0].values.map((row) => {
		const m = {};
		cols.forEach((col, i) => m[col] = row[i]);
		m.toolCall = m.tool_call_json ? JSON.parse(m.tool_call_json) : void 0;
		m.fileDiff = m.file_diff_json ? JSON.parse(m.file_diff_json) : void 0;
		return m;
	});
}
async function dbSaveConversation(conv) {
	(await getDb()).run(`INSERT OR REPLACE INTO conversations (id, title, status, created_at, updated_at) VALUES (?,?,?,?,?)`, [
		conv.id,
		conv.title,
		conv.status,
		conv.createdAt,
...
async function dbSaveMessage(conversationId, msg) {
	(await getDb()).run(`INSERT OR REPLACE INTO messages
...
async function dbSaveEmbeddingsBatch(fileId, chunks) {
	const d = await getDb();
	d.run("BEGIN TRANSACTION");
	try {
		d.run("DELETE FROM embeddings WHERE file_id = ?", [fileId]);
		for (const { idx, text, emb } of chunks) d.run(`INSERT OR REPLACE INTO embeddings (id, file_id, chunk_index, chunk_text, embedding_json)
         VALUES (?,?,?,?,?)`, [
			`${fileId}_${idx}`,
			fileId,
			idx,
...
async function dbSearchEmbeddings(queryEmbedding, workspace, topK = 8) {
	const rows = (await getDb()).exec(`SELECT e.chunk_text, e.embedding_json, f.file_path
     FROM embeddings e
...
async function dbSaveIndexedFile(id, workspace, filePath, hash) {
	(await getDb()).run(`INSERT OR REPLACE INTO indexed_files (id, workspace, file_path, content_hash, indexed_at)
     VALUES (?,?,?,?,?)`, [
		id,
		workspace,
		filePath,
...
async function dbGetIndexedFile(workspace, filePath) {
	const rows = (await getDb()).exec("SELECT * FROM indexed_files WHERE workspace = ? AND file_path = ?", [workspace, filePath]);
...
async function dbDeleteIndexedFile(workspace, filePath) {
	const d = await getDb();
	d.run("BEGIN TRANSACTION");
	try {
		const rows = d.exec("SELECT id FROM indexed_files WHERE workspace = ? AND file_path = ?", [workspace, filePath]);
...
async function dbDeleteIndexedPathPrefix(workspace, dirPath) {
	const d = await getDb();
	const dirPrefix = `${dirPath}${node_path.sep}`;
	d.run("BEGIN TRANSACTION");
	try {
		const rows = d.exec(`SELECT id FROM indexed_files
       WHERE workspace = ? AND (file_path = ? OR file_path LIKE ?)`, [
			workspace,
			dirPath,
			`${dirPrefix}%`
		]);
		const ids = rows.length ? rows[0].values.map((v) => String(v[0])) : [];
		if (ids.length) {
...
//#endregion
exports.dbDeleteConversation = dbDeleteConversation;
exports.dbDeleteIndexedFile = dbDeleteIndexedFile;
exports.dbDeleteIndexedPathPrefix = dbDeleteIndexedPathPrefix;
exports.dbFlushNow = dbFlushNow;
exports.dbGetConversations = dbGetConversations;
exports.dbGetIndexedFile = dbGetIndexedFile;
exports.dbGetMessages = dbGetMessages;
exports.dbSaveConversation = dbSaveConversation;
exports.dbSaveEmbeddingsBatch = dbSaveEmbeddingsBatch;
exports.dbSaveIndexedFile = dbSaveIndexedFile;
exports.dbSaveMessage = dbSaveMessage;
exports.dbSearchEmbeddings = dbSearchEmbeddings;
exports.getDb = getDb;
